...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/account/${userId}`)
  ...
  const logout = () => { localStorage.removeItem('rakta_user'); localStorage.removeItem('rakta_token'); window.location.href = '/auth/login'; };
  ...
  return (
    <main className="container-shell py-16">
      ...
      <section className="panel mt-6 p-6"><h2 className="text-xl font-bold text-white">Notifications and delivery</h2><div className="mt-4 space-y-3">{account.notifications.length ? account.notifications.map((notification) => <div key={notification.id} className="border-b border-white/10 pb-3 text-sm"><div className="font-semibold text-white">{notification.title}</div><div className="mt-1 text-slate-300">{notification.message}</div><div className="mt-2 text-xs text-slate-500">{notification.channels?.map((channel) => `${channel.channel}: ${channel.status}`).join(' · ') || 'In-app notification'}</div></div>) : <p className="text-slate-400">No notifications yet.</p>}</div></section>
